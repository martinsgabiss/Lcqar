...
def populatingGrid(dataEmiss,center,xX,yY,xv,yv):  
    ...
    return data
'''
def griddingMCIP(GRIDCRO2D,GRIDDOT2D):
...
def populatingGridTemp3D(dataEmiss,center,emisPar,xX,yY,temporal_disag,METCRO3D,METCRO2D,GRIDCRO2D):   #FIXME
    ...
    for ii in range(0,dataEmiss.shape[0]):
        ...
        print('Source number = ' + str(ii+1) + ' of '+ str(dataEmiss.shape[0]) )
        ...
        for jj in range(0,dataTempo.shape[1]):
            ...
            lista.append(str(sum(factor)))# FIXME
            
            temp_factor = temporal_disag.iloc[ii, jj]
            ...
            for kk in range (0,dataEmiss.shape[1]):
                dataTempo[kk,jj,:,mindist[0][0],mindist[1][0]]= \
                    np.nansum([dataTempo[kk,jj,:,mindist[0][0],mindist[1][0]],
                               dataEmiss.iloc[ii,kk]*factor*temp_factor],0)   
                
                
                    
                #print(sum(factor)*dataEmiss.iloc[ii,kk]-dataEmiss.iloc[ii,kk])# FIXME
                #print(sum(factor)*dataEmiss.iloc[ii,kk]/dataEmiss.iloc[ii,kk])# FIXME
                lista_0.append(sum(factor)*dataEmiss.iloc[ii,kk]-dataEmiss.iloc[ii,kk]) # FIXME
                lista_1.append(sum(factor)*dataEmiss.iloc[ii,kk]/dataEmiss.iloc[ii,kk]) # FIXME
        dic[ii] = lista # FIXME
    return dataTempo
