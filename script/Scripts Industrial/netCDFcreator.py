...
"""
----------------------netCDFcreator-----------------------------
Created on Fri Apr 30 19:45:06 2021

ESta função cria o arquivo netCDF para o CMAQ. 
Funções:
    createNETCDF - cria o arquivo netCDF anual
    
    createNETCDFtemporal(folder, name, data, xv, yv, lat, lon, center, dates, month)
        Cria o netCDF para as emissões veiculares
        
    createNETCDFtemporalFINN(folder, name, data, xv, yv, lat, lon, dates, month)
        Cria o netCDF para as emissões do FINN
        
Inputs:
    folder: pasta para a criação do output
    name: nome do arquivo a ser criado
    data: matriz de dados para ser gerado o netCDF (cuidado com as dimensões)
    xv, yv: variáveis criadas pelo meshgrid a partir de lat e lon
    center: centroide das emissões
    year: ano para a geração do arquivo
    month: mês da geração do arquivo
...
#%% Creating a dataset
'''
def createNETCDFannual(rootPath,folder,name,data,xX,yY):
    ...
    polid = pd.read_csv(rootPath+'/IndustrialSpeciation/IND_speciation.csv')
    ...
    for ii in range(0,polid.shape[0]):
        ...
        globals()[polid.ID[ii]].units = polid.units[ii]
  
    
    f2.close()
    print('Your annual emissions in netCDF file is ready!')
    '''

def createNETCDFtemporalAndLayered_netcdf3(rootPath,folder,name,data,mcipMETCRO3Dpath,colunasX):
    print('===================STARTING netCDFcreator_v1.py=======================')
    ...
    # f2.NCOLS = f2.NCOLS
    # f2.NROWS = f2.NROWS
    f2.FILEDESC= 'Industrial emissions in KG/YEAR - VARIABLES UNITS ARE WRONG'
    f2.HISTORY =''
    # # Specifying dimensions
    ...
    for ii in range(0,len(colunasX)):
        globals()[colunasX[ii]] = f2.createVariable(colunasX[ii], np.float32, ('TSTEP', 'LAY', 'ROW','COL'))
        globals()[colunasX[ii]][:,:,:,:] = data[ii,:,:,:,:]
        globals()[colunasX[ii]].var_desc = colunasX[ii]+'[1]'
        globals()[colunasX[ii]].units = 'g/s for MP or moles/s for gas'
    ...
    f2.close()
    print('Industrial emissions in netCDF format is ready for CMAQ!')

    #%% Creating a dataset
def createNETCDFtemporalAndLayered(rootPath,folder,name,data,mcipMETCRO3Dpath,colunasX):
    print('===================STARTING netCDFcreator_v1.py=======================')
    ...
    f2.FTYPE =  1
    ds3 = nc4.Dataset(mcipMETCRO3Dpath)
    LAY = ds3.dimensions["LAY"].size
    ROW = ds3.dimensions["ROW"].size
    COL = ds3.dimensions["COL"].size
    for gatr in ds3.ncattrs() :
        setattr(f2, gatr, ds3.getncattr(gatr))
    # f2.VGTYP= -9999
    #f2.VGTOP= 0.0
    ...
    # f2.NCOLS = f2.NCOLS
    # f2.NROWS = f2.NROWS
    f2.FILEDESC= 'Industrial emissions in g/s'
    f2.HISTORY =''
    # # Specifying dimensions
    #tempgrp = f.createGroup('vehicularEmissions_data')
    f2.createDimension('TSTEP', None )
    ...
    varlist = "".join([p.ljust(16) for p in colunasX])
    setattr(f2, "VAR-LIST", varlist)
    
    # Passing data into variables
    TFLAG = f2.createVariable(
        "TFLAG",
        "i4",
        ("TSTEP", "VAR", "DATE-TIME"),
        zlib=True,
    ...
    baseflag = ds3.variables["TFLAG"][:, 0, :]
    TFLAG[:, :, :] = np.repeat(baseflag[:, np.newaxis,:], len(colunasX), axis=1)
        
    vars_nc = {}

    for pol in colunasX:
        vars_nc[pol] = f2.createVariable(
            pol,
            "f4",
            ("TSTEP","LAY","ROW","COL"),
            zlib=True,
            complevel=4,
            chunksizes=(1,LAY,ROW,COL))    
    
    for p,pol in enumerate(colunasX):
        f2.variables[pol][:,:,:,:] = data[p][:,:,:,:]

    f2.sync()
    f2.close()
    print('Industrial emissions file in netCDF format is ready for CMAQ!')
